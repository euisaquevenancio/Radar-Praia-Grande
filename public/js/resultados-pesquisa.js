...
async function pesquisarNoticias(busca, pagina = 1) {
  try {
    const res = await fetch(`/noticia/pesquisar-noticias?busca=${encodeURIComponent(busca)}&pagina=${pagina}`, { credentials: "include" });
    if (!res.ok) {
...
async function pesquisarUsuarios(busca, pagina = 1) {
  try {
    const res = await fetch(`/usuario/pesquisar-usuarios?busca=${encodeURIComponent(busca)}&pagina=${pagina}`, { credentials: "include" });
    const data = await res.json();
