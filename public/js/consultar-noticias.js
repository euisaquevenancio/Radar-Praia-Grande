...
async function capturarNoticiasAdmin(pagina = 1) {
  try {
    const res = await fetch(`/admin/capturar-noticias?pagina=${pagina}`, { credentials: "include" });
...
async function capturarDenunciasNoticiaAdmin(idNoticia, paginaDenunciasNoticia = 1) {
  try {
    const res = await fetch(
      `/admin/capturar-denuncias-noticia/${encodeURIComponent(idNoticia)}/${encodeURIComponent(paginaDenunciasNoticia)}`, { credentials: "include" }
    );
...
async function pesquisarNoticiasAdmin(busca, pagina = 1) {
  try {
    const res = await fetch(`/admin/pesquisar-noticias?busca=${encodeURIComponent(busca)}&pagina=${pagina}`, { credentials: "include" });
    const data = await res.json();
